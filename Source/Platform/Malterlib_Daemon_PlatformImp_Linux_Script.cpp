...
namespace NMib
{
	namespace NService
	{
		...
		NStr::CStr CScript::fp_GetScriptDirectory(EServiceMode _Mode) const
		{
				return mp_ScriptDirectory;
		}
			
		NStr::CStr CScript::fp_GetScriptPath(CServiceParams const &_Params) const
		{
		...
		bool CScript::fp_IsScriptThisExecutable(CServiceParams const &_Params) const
		{
		...
		EActionResult CScript::f_Start(CServiceParams const &_Params)
		{
			...
			
			{
				...
				if (!ExitCode)
				{
					mp_pOwner->f_ReportInformation("Start service", "Service already running");
		...
		EActionResult CScript::f_Stop(CServiceParams const &_Params, bint _bWait)
		{
			if (!fp_CheckParamsSupported(_Params))
				return EActionResult_Failure;
			...
			if (!bServiceExists)
			{
				mp_pOwner->f_ReportInformation("Stop Service", NStr::CStr::CFormat("Service is not installed at '{}' so it has not been stopped") << ScriptFilePath);
				return EActionResult_Success;
			}
		...
		NStr::CStr CScript::fp_CreateScriptFromParams(CServiceParams const &_Params) const
		{
			...
			NStr::CStr PidFilePath = NStr::CStr::CFormat("/var/run/{0}/{0}.pid") << _Params.f_GetServiceName();
...
*/
		...
		EActionResult CScript::fp_AddToRunlevels(CServiceParams const &_Params) const
		{
			NStr::CStr Result;
			NStr::CStr Error;
			uint32 ExitCode = 0;
			...
			if (mp_ScriptRegistrationMethod == EScriptRegistrationMethod_ChkConfig)
			{
					...
					|| ExitCode)
				{
					...
					mp_pOwner->f_ReportInformation("Add Service", NStr::CStr::CFormat("Successfully added {} to runlevels {}") << fs_GetScriptFilename(_Params)  << DInitRunLevels);
			}
			else if (mp_ScriptRegistrationMethod == EScriptRegistrationMethod_UpdateRcD)
			{
					...
					|| ExitCode)
				{
					...
					mp_pOwner->f_ReportInformation("Add Service", NStr::CStr::CFormat("Successfully added {} to default runlevels") << fs_GetScriptFilename(_Params));
			}

			return EActionResult_Success;
		}
		
		EActionResult CScript::fp_RemoveFromRunlevels(CServiceParams const &_Params) const
		{
			NStr::CStr Result;
			NStr::CStr Error;
			uint32 ExitCode = 0;
			...
			if (mp_ScriptRegistrationMethod == EScriptRegistrationMethod_ChkConfig)
			{
					...
					|| ExitCode)
				{
					...
					mp_pOwner->f_ReportInformation("Remove Service", NStr::CStr::CFormat("Successfully removed {} from runlevels {}") << fs_GetScriptFilename(_Params)  << DInitRunLevels);
			}
			else if (mp_ScriptRegistrationMethod == EScriptRegistrationMethod_UpdateRcD)
			{
					...
					|| ExitCode)
				{
					...
					mp_pOwner->f_ReportInformation("Remove Service", NStr::CStr::CFormat("Successfully removed {} from default runlevels") << fs_GetScriptFilename(_Params));
			}

			return EActionResult_Success;
		}

		EActionResult CScript::f_Add(CServiceParams const &_Params, bint _bCheckForExisting)
		{
			if (!fp_CheckParamsSupported(_Params))
				return EActionResult_Failure;
			...
			if (mp_ScriptRegistrationMethod != EScriptRegistrationMethod_None)
			{
				if (fp_AddToRunlevels(_Params) == EActionResult_Failure)
					...
					mp_pOwner->f_ReportInformation("Add Service", NStr::CStr::CFormat("Please add {} to any desired runlevel init scripts manually.") << ScriptFilePath);
				
			return EActionResult_Success;
		}

		EActionResult CScript::f_Remove(CServiceParams const &_Params)
		{
			if (!fp_CheckParamsSupported(_Params))
				return EActionResult_Failure;
			...
			if (!bServiceExists)
			{
				mp_pOwner->f_ReportInformation("Remove Service", NStr::CStr::CFormat("Service is not installed at '{}' so it has not been removed") << ScriptFilePath);
				return EActionResult_Success;
			}
			...
			if (mp_ScriptRegistrationMethod != EScriptRegistrationMethod_None)
			{
				...
				mp_pOwner->f_ReportInformation("Remove Service", NStr::CStr::CFormat("Please remove {} from all runlevel init scripts manually.") << ScriptFilePath);
