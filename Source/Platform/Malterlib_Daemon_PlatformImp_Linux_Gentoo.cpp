...
namespace NMib
{
	namespace NService
	{
		static NStr::CStr fs_GetStartStopDaemonParams(CServiceParams const &_Params)
		{
			NStr::CStr Result;
		...
		static NStr::CStr fs_GetScriptFilename(CServiceParams const &_Params)
		{
			return _Params.f_GetServiceName();
		}
		
		NStr::CStr CGentoo::fp_GetScriptDirectory(EServiceMode _Mode) const
		{
			return mp_ScriptDirectory;
		}
			
		NStr::CStr CGentoo::fp_GetScriptPath(CServiceParams const &_Params) const
		{
			NStr::CStr ScriptFileDirectory = fp_GetScriptDirectory(_Params.f_GetServiceMode());
			return NStr::CStr::CFormat("{}/{}") << ScriptFileDirectory << fs_GetScriptFilename(_Params);
		}

		bool CGentoo::fp_IsScriptThisExecutable(CServiceParams const &_Params) const
		{
			...
			while (!Contents.f_IsEmpty())
			{
				...
				if (nParsed == 1)
				{
					...
					{
						mp_pOwner->f_ReportError(NStr::CStr::CFormat("Service name is already in use: {} and '{}' != '{}'") << ScriptFilePath << Value << ExpectedExec);
						return false;
					}
				}
			}
		...
		CGentoo::EGentooServiceStatus CGentoo::fp_GetStatus(NStr::CStr const &_ScriptFilePath)
		{
			...
			{
				mp_pOwner->f_ReportError(NStr::CStr::CFormat("Error checking running status of service {}: {}\n{}") << _ScriptFilePath << ListResult << ListError);
				return EGentooServiceStatus_InvalidService;
			}
			else
				return (CGentoo::EGentooServiceStatus)ExitCode;
		}
		
		bool CGentoo::fp_GetStatusAdded(NStr::CStr const &_ServiceName)
		{
			NStr::CStr ListResult;
			...
			NStr::CStr ServiceNeedle = NStr::CStr::CFormat("{} | default") << _ServiceName;
			return ListResult.f_FindNoCase(ServiceNeedle) != -1;
		}

		EActionResult CGentoo::f_Start(CServiceParams const &_Params)
		{
			...
			try
			{
				...
				{
					mp_pOwner->f_ReportError(NStr::CStr::CFormat("Service file does not exist: {}") << ScriptFilePath);
					return EActionResult_Failure;
				}
			}
			catch (NFile::CExceptionFile const &_Exception)
			{
				mp_pOwner->f_ReportError(NStr::CStr::CFormat("Failed to check for existing service file: {}") << _Exception.f_GetErrorStr());
				return EActionResult_Failure;
			}

			if (fp_GetStatus(ScriptFilePath) == EGentooServiceStatus_Started)
			{
				mp_pOwner->f_ReportInformation("Start service", NStr::CStr::CFormat("Service was already running: {}") << ScriptFilePath);
			...
			mp_pOwner->f_ReportInformation("Start service", NStr::CStr::CFormat("Service {} was successfully started") << ScriptFilePath);
			return EActionResult_Success;
		}

		EActionResult CGentoo::f_Stop(CServiceParams const &_Params, bint _bWait)
		{
			if (!fp_CheckParamsSupported(_Params))
				return EActionResult_Failure;
			...
			if (!bSerivceExists)
			{
				mp_pOwner->f_ReportInformation("Stop Service", NStr::CStr::CFormat("Service is not installed at '{}' so it was not stopped") << ScriptFilePath);
				return EActionResult_Success;
			}
			...
			if (fp_GetStatus(ScriptFilePath) == EGentooServiceStatus_Stopped)
			{
				mp_pOwner->f_ReportInformation("Stop service", NStr::CStr::CFormat("Service was not running: {}") << ScriptFilePath);
				return EActionResult_Success;
			}
		...
		NStr::CStr CGentoo::fp_CreateScriptFromParams(CServiceParams const &_Params) const
		{
			...
			NStr::CStr PidFileDirectory = NStr::CStr::CFormat("/var/run/{}") << _Params.f_GetServiceName();
			NStr::CStr PidFilePath = NStr::CStr::CFormat("{}/{}") << PidFileDirectory << _Params.f_GetServiceName();
			
			Script += NStr::CStr::CFormat("DESC={}\n") << NMib::NStr::fg_StrEscapeBashSingleQuotes(_Params.f_GetServiceDisplayName());
			Script += NStr::CStr::CFormat("DAEMON={}\n") << NMib::NStr::fg_StrEscapeBashDoubleQuotes(_Params.f_GetExecutablePath());
			Script += NStr::CStr::CFormat("NAME={}\n") << NMib::NStr::fg_StrEscapeBashSingleQuotes(_Params.f_GetServiceName());
			Script += NStr::CStr::CFormat("DAEMON_ARGS=( '-Service' {} '-Daemonize' {} )\n") << NMib::NStr::fg_StrEscapeBashSingleQuotes(_Params.f_GetServiceName()) << NMib::NStr::fg_StrEscapeBashSingleQuotes(PidFilePath);
...
*/
		...
		EActionResult CGentoo::f_Add(CServiceParams const &_Params, bint _bCheckForExisting)
		{
			if (!fp_CheckParamsSupported(_Params))
				return EActionResult_Failure;
			...
			NStr::CStr ServiceName = fs_GetScriptFilename(_Params);
			...
			{
				mp_pOwner->f_ReportError(NStr::CStr::CFormat("Failed to install service at {}: {}") << ScriptFilePath << _Exception.f_GetErrorStr());
				return EActionResult_Failure;
			}
			...
			if (!NMib::NProcess::CProcessLaunch::fs_LaunchBlock(DRCUpdateBin, NStr::CStr::CFormat("add {} default") << ServiceName, Result, Error, ExitCode)
				|| ExitCode
				|| !Error.f_IsEmpty())
			{
				mp_pOwner->f_ReportError(NStr::CStr::CFormat("Failed to add service {} to runlevel default: {}") << ServiceName << Error);
				return EActionResult_Failure;
			}
			
			mp_pOwner->f_ReportInformation("Add Service", NStr::CStr::CFormat("Successfully installed service at {}") << ScriptFilePath);
			return EActionResult_Success;
		}

		EActionResult CGentoo::f_Remove(CServiceParams const &_Params)
		{
			if (!fp_CheckParamsSupported(_Params))
				return EActionResult_Failure;
			...
			if (fp_GetStatusAdded(ServiceName))
			{
				NStr::CStr Result;
				NStr::CStr Error;
				uint32 ExitCode = 0;
				
				if (!NMib::NProcess::CProcessLaunch::fs_LaunchBlock(DRCUpdateBin, NStr::CStr::CFormat("del {} default") << ServiceName, Result, Error, ExitCode)
					|| ExitCode
					|| !Error.f_IsEmpty())
				{
					mp_pOwner->f_ReportError(NStr::CStr::CFormat("Failed to remove service {} from runlevel default: {}") << ServiceName << Error);
					return EActionResult_Failure;
				}

				mp_pOwner->f_ReportInformation("Remove Service", NStr::CStr::CFormat("Successfully removed service {} from runlevel default") << ServiceName);
			}
			else
				mp_pOwner->f_ReportInformation("Remove Service", NStr::CStr::CFormat("Service {} did not exist in runlevel default") << ServiceName);
			...
			if (!bSerivceExists)
			{
				mp_pOwner->f_ReportInformation("Remove Service", NStr::CStr::CFormat("Service is not installed at '{}' so it was not removed") << ScriptFilePath);
				return EActionResult_Success;
			}
