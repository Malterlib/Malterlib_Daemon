	...
	namespace NService
	{
		...
		public:
			CDetails(CService* _pOwner)
				: mp_pOwner(_pOwner)
				, mp_InterruptedReason(EServiceInterrupt_NotInterrupted)
			{
				if (msp_pThis)
				...
				if (CUpstart::fs_IsSupported())
				{
					DMibLog(Debug, "Upstart service supported.", 0);
					mp_pServiceIntegration = fg_Construct<CUpstart>(_pOwner);
				}
				else if (CSystemd::fs_IsSupported())
				{
					DMibLog(Debug, "Systemd service supported.", 0);
					mp_pServiceIntegration = fg_Construct<CSystemd>(_pOwner);
				}
				else if (CGentoo::fs_IsSupported())
				{
					DMibLog(Debug, "Gentoo service supported.", 0);
					mp_pServiceIntegration = fg_Construct<CGentoo>(_pOwner);
				}
				else if (CScript::fs_IsSupported())
				{
					DMibLog(Debug, "Script service supported.", 0);
					mp_pServiceIntegration = fg_Construct<CScript>(_pOwner);
				}
				else
				{
					mp_pOwner->f_ReportError(NStr::CStr::CFormat("No supported service system detected."));
				}
			}
			...
			bool f_PrepareUserAndGroup(CServiceParams const &_Params)
			{
				...
				NStr::CStr PIDDirectory = NStr::CStr::CFormat("/var/run/{}") << _Params.f_GetServiceName();
				try
				{
			...
			{
				return mp_pServiceIntegration->f_Start(mp_pOwner->mp_Params);
			}
			...
			{
				return mp_pServiceIntegration->f_Exists(mp_pOwner->mp_Params, _bExists);
			}
			...
			EActionResult f_Add(bool _bCheckForExisting)
			{
				...
				return mp_pServiceIntegration->f_Add(mp_pOwner->mp_Params, _bCheckForExisting);
			}
			
			EActionResult f_Remove()
			{
				return mp_pServiceIntegration->f_Remove(mp_pOwner->mp_Params);
			}
			...
			static void fs_SigHandler(int const _sigid)
			{
				...
				{
					case SIGTSTP:
						msp_pThis->mp_InterruptedReason = EServiceInterrupt_Pause;
						break;
					case SIGCONT:
						msp_pThis->mp_InterruptedReason = EServiceInterrupt_Resume;
						break;
					default:
						msp_pThis->mp_InterruptedReason = EServiceInterrupt_Exit;
						break;
				}
			...
			int fp_DoStatus(NStr::CStr const &_PidFilePath)
			{
				...
				if (PidOfProcess)
				{
					if (NMib::NProcess::NPlatform::fg_Process_IsRunning(PidOfProcess))
					{
						mp_pOwner->f_ReportInformation("Service Status", NStr::CStr::CFormat("{} ({}) is running...") << mp_pOwner->mp_Params.f_GetExecutablePath() << PidOfProcess);
						return 0;
					}
					mp_pOwner->f_ReportInformation("Service Status", NStr::CStr::CFormat("{} is stopped (pid file still exists)") << mp_pOwner->mp_Params.f_GetExecutablePath());
					return 1;
				}

				mp_pOwner->f_ReportInformation("Service Status", NStr::CStr::CFormat("{} is stopped") << mp_pOwner->mp_Params.f_GetExecutablePath());
			...
			EActionResult f_Run()
			{
				...
				if (mp_pOwner->mp_Params.f_GetDaemonize())
				{
					...
					Params.m_Parameters = NStr::CStr::CFormat("-Service {} --OutputPID") << mp_pOwner->mp_Params.f_GetServiceName();
				...
				{
					{
						if (!mp_pOwner->mp_Params.f_GetRunAsUser().f_IsEmpty() || !mp_pOwner->mp_Params.f_GetRunAsGroup().f_IsEmpty())
						{
							NMib::NProcess::CProcessLaunchParams Params;
							Params.m_Target = mp_pOwner->mp_Params.f_GetExecutablePath();
							Params.m_Parameters = NStr::CStr::CFormat("-Service {}") << mp_pOwner->mp_Params.f_GetServiceName();
							...
							while(!bExit)
							{
								...
								switch (mp_InterruptedReason.f_Exchange(EServiceInterrupt_NotInterrupted))
								{
									case EServiceInterrupt_Pause:
										kill(Launcher.f_GetProcessID(), SIGTSTP);
										break;
									case EServiceInterrupt_Resume:
										kill(Launcher.f_GetProcessID(), SIGCONT);
										break;
									case EServiceInterrupt_Exit:
		...
		private:
			...
			NPtr::TCUniquePointer<CServiceSystemInterface> mp_pServiceIntegration;
			bool mp_bDaemonize;
		
			enum EServiceInterrupt
			{
				EServiceInterrupt_NotInterrupted = 0,
				EServiceInterrupt_Exit,
				EServiceInterrupt_Pause,
				EServiceInterrupt_Resume,
			};
			NAtomic::TCAtomic<EServiceInterrupt> mp_InterruptedReason;
		...
		namespace
		{
			...
			{
				EServiceFeature m_SupportedFeatures;
				CCalcFeatures()
				{
					m_SupportedFeatures = EServiceFeature_GlobalService;
					try
					{
						if (NMib::NFile::CFile::fs_FileExists(NStr::CStr("/etc/redhat-release")))
						{
							...
							if (Contents.f_StartsWith("Fedora release 18 ")
								...
								m_SupportedFeatures |= EServiceFeature_EscapedPathBroken;
							else if (Contents.f_StartsWith("CentOS release 6.")
								|| Contents.f_StartsWith("Red Hat Enterprise Linux Server release 6."))
								m_SupportedFeatures |= EServiceFeature_EscapeCharBroken;	// Upstart escape character is broken
						}
					}
			...
			NAggregate::TCAggregate<CCalcFeatures> g_SupportedFeatures = {DAggregateInit};
		}
		...
		void CService::f_ReportInformation(NStr::CStr const& _Heading, NStr::CStr const& _Message)
		{
			f_GetServiceParams().f_ReportInformation(_Heading, _Message);
		}
		
		void CService::f_ReportError(NStr::CStr const& _Message)
		{
			f_GetServiceParams().f_ReportError(_Message);
		}
		
		EReportError CService::f_ReportErrorYesNo(NStr::CStr const& _Message, EReportError _Default)
		{
